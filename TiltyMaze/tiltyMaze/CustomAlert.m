...
- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id)AlertDelegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle
{
    ...
    if (self) {
        ...
        if (message && ![message isEqualToString:@"TextLabel"])
        {
        ...
        }else if([message isEqualToString:@"TextLabel"]){
            float max_msg_height = MAX_ALERT_HEIGHT - alert_height - ((cancelButtonTitle || otherButtonTitle)?(CancelBtnImg.size.height+30.0):30.0);
            
            self.textValue = [[UITextField alloc] initWithFrame:CGRectMake(10.0, 0.0, alert_width-40.0, 0.0)];
            [self.textValue setDelegate:self];
            self.textValue.font = [UIFont systemFontOfSize:16.0];
            [self.textValue setBackgroundColor:[UIColor whiteColor]];
            
            [self.textValue sizeToFit];
            self.textValue.frame = CGRectMake(10.0, 0.0, alert_width-40.0, self.textValue.frame.size.height);
            
            while (self.textValue.frame.size.height>max_msg_height && self.textValue.font.pointSize>12) {
                self.textValue.font = [UIFont systemFontOfSize:self.textValue.font.pointSize-1];
                [self.textValue sizeToFit];
                self.textValue.frame = CGRectMake(10.0, 0.0, alert_width-40.0, self.textValue.frame.size.height);
            }
            
            MsgScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(10.0, alert_height, alert_width-20.0, (self.textValue.frame.size.height>max_msg_height)?max_msg_height:self.textValue.frame.size.height)];
            MsgScrollView.contentSize = self.textValue.frame.size;
            [MsgScrollView addSubview:self.textValue];
        ...
        //add background
        if ([message isEqualToString:@"TextLabel"]) {
            AlertView = [[UIView alloc] initWithFrame:CGRectMake((int)((self.frame.size.width-alert_width)/2.0), (int)((self.frame.size.height-alert_height)/2.0) - 90, alert_width, alert_height)];
        }else{
